...
for (var y = 0, lenX = matrix[0].length, lenY = matrix.length; y < lenY; y++) {
  ...
  // Move columns of numbers in rows
  for (var x = 0; x < lenX; x++) {
    columns[(lenX - 1 - x)].push(matrix[y][x]);
  }
...
for (var i = 0, len = columns.length; i < len; i++) {
  columns[i] = columns[i].reduce(function(a, b) { return a + b; });
}

columns.push(0);

for (var i = 0, len = columns.length - 1, rem = 0; i < len; i++) {

  // Get remainder and add to next number
  columns[i + 1] += Math.floor((columns[i] / 10));

  // Remove the remainder from original number
  columns[i] = columns[i] % 10;

}

// Reverse order and join numbers
result = columns.reverse().join('').slice(0, 10);
